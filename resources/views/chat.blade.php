                    ...
                    <ul class="conversation-list">
                        @foreach ($conversations as $conv)
                            <li class="mb-2 flex items-center justify-between text-white">
                                <a href="{{ route('chat.show', $conv) }}" class="block p-2 rounded flex-grow text-white {{ $currentConversation && $currentConversation->id == $conv->id ? 'bg-blue-100 text-black' : 'hover:bg-gray-100 text-black ' }}">
                                    {{ $conv->title }}
                                </a>
